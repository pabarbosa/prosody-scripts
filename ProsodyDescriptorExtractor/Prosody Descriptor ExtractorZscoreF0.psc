# Prosody Descriptor Extractor_v.F0norm.psc
...
# Copyright (C) 2021 Barbosa, P. A. &  G. Silveira (Modification for F0 normalization)
...
#  New fonctionalities
#
# Date: 2012, 2015, new version (3.0): Nov 2021.
...
# f0 descriptors and f0 rate (tonerate) computation per chunk
 ...
 Down to PitchTier
 if inSemitones
  f0mean = Get mean... 'initime' 'endtime' semitones
  f0sd = Get standard deviation... 'initime' 'endtime' semitones
  Formula... (self-f0mean)/f0sd
  Down to TableOfReal: "Hertz"
  To Table: "rowLabel"
  f0median = Get quantile: "F0", 0.5
  f0sd = Get standard deviation: "F0"
  f0min = Get quantile: "F0", 0.01
  f0max = Get quantile: "F0", 0.99
  f0base = f0median - 1.43*f0sd
  f01Q = Get quantile: "F0", 0.25
  f03Q = Get quantile: "F0", 0.75
  f0SAQ = (f03Q-f01Q)/2
 else
  f0mean = Get mean (points): 0.0, 0.0
  f0sd = Get standard deviation (points): 0.0, 0.0
  Formula... (self-f0mean)/f0sd
  Down to TableOfReal: "Hertz"
  To Table: "rowLabel"
  f0median = Get quantile: "F0", 0.5
